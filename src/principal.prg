...
Function Main()
   ...
   *
   ...
   DEFINE WINDOW oWnd TITLE SIS_NAME STYLE nOr( WS_POPUP, WS_MAXIMIZE );
    COLOR CLR_WHITE, CLR_GREEN
   *
   DEFINE METROPANEL oMetro OF oWnd TITLE SIS_NAME+" "+SIS_VERSAO+IIF( Empty(nomeDoUsuario), ""," Bem-vindo "+nomeDoUsuario) ;
    COLOR CLR_WHITE, CLR_BLACK
   *
   oMetro:lDesignMode := .F.
   * oMetro:SetBackGround('.\RES\SLIDEIMAGE.BMP')
   *
		@ 10,40 BTNBMP oBtnExit RESOURCE "EXIT" ;
   ...
   MsgRun("Carregando botoes...","Aguarde...",{||CarregaBotoes(.f.)})
   *@ 40,230 SAY oTitulo Prompt SIS_NAME+" "+SIS_VERSAO+IIF( Empty(nomeDoUsuario), ""," Bem-vindo "+nomeDoUsuario) OF oMetro Font oTextFont2 PIXEL SIZE 1500,50 TRANSPARENT
   *
   //oMetro:bPainted:={||PintarBotoes()}
	ACTIVATE WINDOW oWnd MAXIMIZED ON INIT (BuildTimer(oWnd),Cursor(),oMetro:Show()) 
FUNCTION PintarBotoes()
	   *
//	   nLinhaPadrao:=(oMetro:nBtnSize - oMetro:oFont:nHeight ) / 2
//	   nColunPadrao:=oMetro:nMetroTop
		
//		nLinhaPadrao_Exit:=nLinhaPadrao-13
//	   nColunPadrao_Exit:=nColunPadrao-122

//	   nLinhaPadrao_Logo:=nLinhaPadrao-47
//	   nColunPadrao_Logo:=nColunPadrao
	   
//		FWDBG nLinhaPadrao,nColunPadrao,nLinhaPadrao_Exit,nColunPadrao_Exit,nLinhaPadrao_Logo,nColunPadrao_Logo
//		oBtnLogo:Refresh()
//		oBtnExit:Refresh()
		*/
		*SysRefresh()
...
function BuildTimer( oWindow)

   local oTmr
   
   DEFINE TIMER oTmr OF oWindow ;
    ACTION ( MsgWait("Programa será encerrado, tempo finalizado"), __Quit() ) INTERVAL 1080000
...
Function CarregaBotoes(lDestroi)
   Default lDestroi:=.f.
   if lDestroi
   ...
   if temSenha
      ...
      *aadd(aBotoes,oBtnImportar)
   endif
	
   select reserva
   go top
   while ! reserva->(eof())
      cBtn:= "oBtn"+strzero(reserva->id,5,0)
      *
...
Function Reservar(nIdReserva)
   ...
   *
   *if !lStatus
      *	MsgAlert("Reserva ocupada até "+dtoc(dDataInicio)+" às "+hHoraInicio,"Atenção")
      *	return
   *endif
   *
   ...
   if lReturn
      ...
      if nDia > 0
         ...
         While !Eof() .and. dispor->idreserva == nIdReserva
				if dispor->dia == nDia
               lExiste:=.t.
               if !Empty(dispor->limite)
						if ALLTRIM(ElapTime(hhoraini,hhorafim)) > ALLTRIM(dispor->limite)
					...
					if dispor->hrini <= hhoraini .and. hhorafim <= dispor->hrfim
                  lHorario:=.t.
                  exit
               endif
            endif
...
function Login()
   ...
   ACTIVATE DIALOG oDlg CENTERED ON INIT (BringWindowToTop( oDlg:hWnd ),BuildTimer( oDlg)) VALID (IIF(lOk,Login_Senha(cUserName,cPassword),.T.))
